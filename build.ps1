...
Write-Host "`n[1/4] Cleaning build dir..." -ForegroundColor Cyan
if (Test-Path $tmp) { Remove-Item $tmp -Recurse -Force }

Write-Host "[2/4] Copying project to $tmp..." -ForegroundColor Cyan
# Copy only source files (skip target dir)
...
New-Item -ItemType Directory -Path "$tmp\ui" -Force | Out-Null
...
Write-Host "[4/4] Copying installers back..." -ForegroundColor Cyan
